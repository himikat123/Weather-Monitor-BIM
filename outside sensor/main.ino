...
void setup() {
  ...
  if(html.ssid==""){
    ...
    while(WiFi.status()!=WL_CONNECTED){
      if(e&1) strip.setPixelColor(0,strip.Color(150,150,0));
      else{
        if(bat_level==1) strip.setPixelColor(0,strip.Color(150,0,0));
        else strip.setPixelColor(0,strip.Color(150,0,0));
      }
      strip.show(); 
      if((e++)>50){
