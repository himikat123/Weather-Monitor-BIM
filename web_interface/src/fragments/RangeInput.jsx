...
class RangeInput extends Save {
     ...
     */
    changedValue(event) {
        let config = this.setVal(this.props.config, this.props.value, +((+event.target.value).toFixed(this.dec(this.props.step))));
    ...
    minus(event) {
        ...
        let val = +(this.getVal(this.props.config, this.props.value));
        if(val > +(this.props.limitMin)) val -= +(this.props.step);
        let config = this.setVal(this.props.config, this.props.value, +(val.toFixed(this.dec(this.props.step))));
        this.changedConfig(config);
        if(this.props.callback) this.props.callback();
    }
    ...
    plus(event) {
        event.preventDefault();
        let val = +(this.getVal(this.props.config, this.props.value));
        if(val < +(this.props.limitMax)) val += +(this.props.step);
        let config = this.setVal(this.props.config, this.props.value, +(val.toFixed(this.dec(this.props.step))));
        this.changedConfig(config);
        if(this.props.callback) this.props.callback();
    }
